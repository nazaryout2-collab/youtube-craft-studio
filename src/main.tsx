...
import { ArrowDown, ArrowUpRight, AudioLines, Bot, Captions, Check, Film, Images, Menu, Sparkles, Users, Wand2, X, Zap } from "lucide-react";
import "./index.css";
...
function Originality() { return <section className="originality"><VideoBackground src={videos.studio} dark={.78} /><Reveal className="originality-title"><span className="section-no">[ 03 / АВТОРСКИЙ КОНТРОЛЬ ]</span><h2>НЕ КОНТЕНТ-ФАБРИКА.<br /><em>ТВОЯ СТУДИЯ.</em></h2></Reveal><Reveal className="originality-note"><Sparkles /><p><b>Программа не заменяет автора.</b> Она ускоряет его руки. Вы управляете сценарием, источниками, визуальным языком и финальным монтажом.</p><p>Оригинальная подача, собственная редактура и ценность для зрителя помогают соответствовать правилам платформ. Решение о монетизации всегда принимает YouTube — ни один софт не может гарантировать её автоматически.</p></Reveal></section>; }
